  ...
  五、优先队列(堆)
    ...
    6.排序
        ...
        6.1.2 简单排序算法(插入、选择)的下界(最坏运行时间)
                 
            定理一： N个元素的数组的平均逆序树(需要交换的次数)为N(N-1)/4。
            
                证明：将一个序列和它反序列的总逆序树为n(n-1)/2,所以平均为N(N-1)/4。
                   
            定理二：通过交换相邻元素的排序的所有算法平均运行时间为 Ω(n^2)。
                    
        
        6.2 希尔排序(增量缩减排序)
        
            6.1.1 描述：希尔排序是第一批冲破N(O^2)排序的算法。它是比较相距一定距离(增量)的两个元素,
                        每一趟的比较相距的距离缩短(缩短增量),直到相距1为最后一趟的比较。影响希尔
                        排序的性能是增量序列的选择。
                        
                  定理一：使用希尔增量序列,时间复杂度为O(n^2),不过在测试中希尔排序的性能很好。
                         希尔增量序列： Δ₁ = array.length / 2 、 Δk = Δk+1 / 2
                   
                  定理二：使用hibbard增量序列,时间复杂度为O(n^(3/2))。
                         hibbard增量序列: 1 3 5 7 ... 2ⁿ - 1 (基数序列)
                                                   
                                                   
        6.3 堆排序
        
            6.3.1 描述：堆排序是一个稳定的算法,以nlog(n)时间排序。将序列构建为一个二叉堆(
                        以原数组为基础,从最后一个父节点进行滤),然后执行的deletelMax操作。
                        deleteMax的这个序列顺序就是排序序列。这里可以使用一个技巧,
                        每次deletelMin的元素放到序列的最后(这样可以节省存储deleteMax的一个新数组),
                        将最后的元素放到根上,再进行堆的下滤(不包括最后序列中已经的deleteMax的子序列)。
                    ...
                    实现：1.第一个元素从数组1开始
                        ...
                        2.第一个元素从数组0开始
                            (0) 根节点 0   
                            (1)最后一个父节点：(length / 2) - 1
                            (2)左节点 2n + 1
                            (3)右节点 2n + 2   
                             
                注意：已经deleteMax的子序列不能在下一次的deleteMax参与下滤。     
            
            6.3.2 性能分析：对于n个元素的数组进行堆排序,最坏进行了2nlog(n) - O(n),
                            平均进行的了2 nlog(n) - O(n log logn)次的比较,
                            如果去除常数,那么平均运行时间为nlog(n)。
